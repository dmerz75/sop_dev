...
#  Import Data! (2/4)                                         #
#  ---------------------------------------------------------  #
result_type = 'lennardjones' # sop | sopnucleo | gsop | namd
plot_type = 'potential' # fe | tension | rmsd | rdf
data_name = 'repulsive' # seed #
...
V = 4 * 1.0 * ((r0 / xlj) ** 12 - (r0 / xlj) ** 6)
plt.plot(xlj,V,'r-',linewidth=7)
...
majorLocator = matplotlib.ticker.MultipleLocator(0.75)
majorLocatorx = matplotlib.ticker.MultipleLocator(0.4)
...
# 2:
# lst_labels = ['','',]
lst_labels = [r'$\epsilon_{l}$ = 1.00']
ax1.legend(lst_labels,loc=1,prop={'size':30})
# 3:
